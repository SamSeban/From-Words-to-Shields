...
def executor(prompt, file_path=None, planner=None):
    ...
    with AuditLog(prompt, file_path) as log:
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                audit.planning_start()
                manifest = planner.plan(prompt)
                audit.planning_end(manifest)
                ...
                if "error" in manifest:
                    raise RuntimeError(f"Planning failed: {manifest['error']}")
                ...
                audit.execution_end(execution_results)
                ...
                if "error" in execution_results:
                    raise RuntimeError(f"Execution failed: {execution_results['error']}")
                ...
                if "pipeline_results" in execution_results:
                    for result in execution_results["pipeline_results"]:
                        if not result["success"]:
                            raise RuntimeError(f"Step {result['step']} ({result['tool']}) failed")
                ...
                if attempt < max_attempts:
                    ...
                    audit.retry(attempt, max_attempts, error_msg)
                    continue
                print(f"ERROR: Workflow failed after {max_attempts} attempts: {error_msg}")
                import traceback
                traceback.print_exc()
                return False
